...
Assuming all the compile should be involved in `int32` type, which means every returned answer should be in [-2^31, 2^31-1]
...
I personally regard the symbol and the value as two divided attributes in a single integer. Thus, use  if(int<0) to check whether the integer is negative is quite useful, we can quickly Multiply -1 to it , and use another variable to save it’s symbol. It’s useful when the problem do not care symbol, for example,  Problem  7: Reverse Integer.

We can simply isolate the symbol, do reverse in whatever way you like , and reduce symbol:
...
However, everything above can be represented as 3 lines with total same thoughts of solution:
...
Some times we need to get each number of a integer, for example (int)1234, we hope we can get 1, 2,3,4 every time, or 4,3,2,1 every time. Next I use Problem 2 : Add Two Numbers as an example.
